...
const emptyLocale = (locale: string): HeroLocale => ({
  locale,
  greeting: "",
  nameLabel: "",
  description: "",
...
const fields: { key: keyof Omit<HeroLocale, "locale">; label: string; textarea?: boolean }[] = [
  { key: "greeting", label: "Greeting" },
  { key: "nameLabel", label: "Name Label" },
  { key: "description", label: "Description", textarea: true },
  { key: "btnProject", label: "Button Project" },
  { key: "btnContact", label: "Button Contact" },
  { key: "status", label: "Status" },
  { key: "stickerExp", label: "Sticker Exp" },
  { key: "stickerOpen", label: "Sticker Open" },
  { key: "availableText", label: "Available Text" },
];

export default function HeroEditor() {
  const [locales, setLocales] = useState<Record<string, HeroLocale>>({
    id: emptyLocale("id"),
    en: emptyLocale("en"),
  });
  ...
  useEffect(() => {
    fetch("/api/admin/hero")
      .then((r) => r.json())
      .then((data: HeroLocale[]) => {
        const map: Record<string, HeroLocale> = {
          id: emptyLocale("id"),
          en: emptyLocale("en"),
        };
        data.forEach((item) => {
  ...
  const update = (locale: string, key: keyof Omit<HeroLocale, "locale">, value: string) => {
    setLocales((prev) => ({
      ...prev,
